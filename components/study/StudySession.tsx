...
export interface SessionResult {
  total: number;
  correct: number;     // ratings 3 or 4
  hard: number;        // ratings 1 or 2
  timeSeconds: number;
}
...
const RATING_CONFIG = [
  { rating: 1 as Rating, label: 'Muy difícil', color: '#dc2626', bg: '#fef2f2', border: '#fecaca', textColor: '#7f1d1d' },
  { rating: 2 as Rating, label: 'Difícil',     color: '#ea580c', bg: '#fff7ed', border: '#fed7aa', textColor: '#431407' },
  { rating: 3 as Rating, label: 'Fácil',       color: '#16a34a', bg: '#f0fdf4', border: '#bbf7d0', textColor: '#14532d' },
  { rating: 4 as Rating, label: 'Muy fácil',   color: '#2563eb', bg: '#eff6ff', border: '#bfdbfe', textColor: '#1e3a8a' },
];
...
export function StudySession({ cards, deckId, deckName, deckColor, userId, onComplete }: StudySessionProps) {
  ...
  const total   = cards.length; // fixed total for progress — not queue length
  ...
  const handleRating = useCallback(async (rating: Rating) => {
    ...
    // Fire-and-forget updates — don't block UI
    updateCard(card.$id, {
    ...
    setTimeout(() => {
      if (rating === 1) {
        // Re-insert hard card 3 positions ahead — but only up to 2 extra retries to avoid infinite loops
        const retryCount = (card as any).__retries || 0;
        if (retryCount < 2) {
          const newQ = [...queue];
          const [c] = newQ.splice(current, 1);
          (c as any).__retries = retryCount + 1;
          newQ.splice(Math.min(current + 3, newQ.length), 0, c);
          setQueue(newQ);
        } else {
          // Gave up on this card — count it and move on
          setDone(d => d + 1);
          setHard(h => h + 1);
          if (done + 1 >= total) {
            onComplete({ total, correct, hard: hard + 1, timeSeconds: Math.round((Date.now() - sessionStart.current) / 1000) });
            return;
          }
          setCurrent(c => c + 1);
        }
        ...
        setDone(newDone);
        if (isHard) setHard(h => h + 1);
        else setCorrect(c => c + 1);

        if (newDone >= total || current + 1 >= queue.length) {
          onComplete({
            total,
            correct: correct + (isHard ? 0 : 1),
            hard: hard + (isHard ? 1 : 0),
  ...
  // Mini progress dots (show up to 20 dots)
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: 'var(--background)' }}>
      ...
      <div className="flex items-center gap-3 px-4 py-3 sm:px-6 sm:py-4 shrink-0"
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-1.5">
            ...
            <span className="text-xs tabular-nums font-medium ml-3 shrink-0"
          ...
          <div className="h-2 rounded-full overflow-hidden" style={{ backgroundColor: 'var(--accent)' }}>
            <div className="h-full rounded-full transition-all duration-500"
              style={{ width: `${pct}%`, backgroundColor: deckColor }} />
          </div>
          {/* Dot progress for small sessions */}
          {showDots && (
            <div className="flex gap-1 mt-1.5 overflow-hidden">
              {Array.from({ length: dotCount }).map((_, i) => (
                <div key={i} className="flex-1 h-1 rounded-full transition-all duration-300"
                  style={{
                    backgroundColor: i < done ? deckColor : i === done ? deckColor + '55' : 'var(--accent)',
                  }} />
              ))}
            </div>
          )}
      ...
      <div className="flex-1 flex flex-col px-4 py-5 sm:px-6 sm:py-8 max-w-2xl mx-auto w-full">
        ...
        {/* Flashcard */}
        <div
          className="flex-1 flex flex-col rounded-2xl border-2 p-6 sm:p-8 mb-5 cursor-pointer select-none transition-all duration-250"
          style={{
        ...
        >
          ...
          <div className="flex-1 flex items-center justify-center">
            <p className="text-lg sm:text-xl font-medium leading-relaxed text-center whitespace-pre-wrap"
